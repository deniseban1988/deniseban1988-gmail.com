...
async function debug() {
  ...
  try {
    ...
    // Reset password again just to be 100% sure
    await auth.updateUser(user.uid, { password: 'Admin123!' });
    console.log('Password re-reset to: Admin123!');

    const doc = await db.collection('users').doc(user.uid).get();
    if (doc.exists) {
      console.log('Firestore Doc Data:', JSON.stringify(doc.data(), null, 2));
      
      // Ensure the document has the correct role and is active
      ...
      console.log('Firestore Doc updated for SUPER_ADMIN role.');
    } else {
