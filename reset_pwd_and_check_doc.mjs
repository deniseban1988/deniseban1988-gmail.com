...
async function run() {
  ...
  await auth.updateUser(user.uid, { password: 'TemporaryPassword123!' });
  console.log('Password reset to: TemporaryPassword123!');
  
  const docRef = db.collection('users').doc(user.uid);
  const snap = await docRef.get();
  if (snap.exists) {
    console.log('User doc in Firestore:', JSON.stringify(snap.data(), null, 2));
  } else {
    console.log('User doc MISSING in Firestore!');
    await docRef.set({
      id: user.uid,
