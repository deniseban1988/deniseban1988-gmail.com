...
if (getApps().length === 0) {
const firebaseProjectIdEnv = process.env.FIREBASE_PROJECT_ID || 'studio-2569273626-e2093';
  
  try {
    if (firebaseProjectIdEnv.startsWith('{')) {
      ...
      console.log(`Firebase Admin initialized with service account for project: ${serviceAccount.project_id} | DB: ${FIRESTORE_DB_ID} (${isProduction ? 'PROD' : 'DEV'})`);
    } else {
      initializeApp({
        projectId: firebaseProjectIdEnv
      });
      console.log(`Firebase Admin initialized with project ID: ${firebaseProjectIdEnv} | DB: ${FIRESTORE_DB_ID} (${isProduction ? 'PROD' : 'DEV'})`);
    }
  } catch (err: any) {
      ...
      return res.json({
        ...
        projectName: (() => {
          try {
            ...
            if (!pid) return 'studio-2569273626-e2093';
            if (pid.startsWith('{')) return JSON.parse(pid).project_id;
            return pid;
          } catch (e) {
            return 'studio-2569273626-e2093';
          }
        })(),
